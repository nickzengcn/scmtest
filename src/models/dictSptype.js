...
export default {
    ...
    effects: {
        *fetch({ payload }, { call, put }) {
            ...
            const requestData = { type: 'QueryReplenSptypeList', data: payload }
        ...
        *editRow({ payload }, { call, put }) {
            const type = payload.ID?'EditReplenSptype':'CreateReplenSptype';
            //请求保存
        ...
        *deleteRow({ payload }, { call, put }) {
            const requestData = { type: 'DeleteReplenSptype', data: payload };
            const response = yield call(getDictListByParames, requestData);
            //重新请求数据
            yield put({
                type: 'fetch',
